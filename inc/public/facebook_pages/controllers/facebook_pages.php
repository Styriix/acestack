<?php
class facebook_pages extends MY_Controller {
	...
	public function index($page = "", $ids = "")
	{
        ...
        try {
            ...
            $response = $this->get('/me/accounts?fields=id,name,username,fan_count,link,is_verified,picture,access_token,category&limit=10000', $accessToken);
            if(is_string($response)){
                $response = $this->get('/me/accounts?fields=id,name,username,fan_count,link,is_verified,picture,access_token,category&limit=3', $accessToken);
            }
	...
	public function oauth()
	{
		...
		$helper = $this->fb->getRedirectLoginHelper();
        $permissions = [ get_option('facebook_page_permissions', 'publish_pages,manage_pages') ];
        	...
        	if(isset($response->data) && !empty($response->data)){
        		foreach ($response->data as $row) {
        			...
        			if(in_array($row->id, $ids, true)){
        				...
        				$avatar = save_img( $row->picture->data->url, TMP_PATH.'avatar/' );
